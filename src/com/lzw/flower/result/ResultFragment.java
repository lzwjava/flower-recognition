...
public class ResultFragment extends Fragment implements View.OnClickListener {
  ...
  private void setBitmapByKey(final String key) {
    new AsyncTask<Void, Void, Void>() {
      ...
      protected Void doInBackground(Void... params) {
        try {
          bm = Web.getBitmapCacheNet(key, w);
          res = true;
        } catch (Exception e) {
          res = false;
          e.printStackTrace();
  ...
  private int getLeftPicWidth() {
    Resources res = getResources();
    int w = res.getDimensionPixelSize(R.dimen.showViewWidth);
    int padW = res.getDimensionPixelSize(R.dimen.showViewPadding) * 2;
    return w - padW;
  }
  ...
  public Bitmap getOriginBitmap() {
    try {
      ...
      e.printStackTrace();
      throw new NullPointerException("origin not set");
    }
  }
